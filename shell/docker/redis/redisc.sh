# docker pull daocloud.io/library/redis:6.2.1
# 李广龙
# 编写时间2022-03-30

echo Redis从机器部署脚本
echo daocloud.io/library/redis版本6.2.1
#服务目录
# 请手动创建服务目录
# 默认容器数据卷挂载目录在服务目录下data文件夹中
SERVICE_PATH="/data/jtmessage/redisc"
SERVICE_NAME="redisc"
PORT="6379"
...
PASSWORD="eagleredis"
#主redisip+端口
SLAVEOF="172.210.1.204 6379"
# 主redis密码
ZPASSWORD="eagleredis"
...
#bind 0.0.0.0
#################################
# 保护模式：不允许外部网络连接redis服务 开启protected-mode保护模式，需配置bind ip或者设置访问密码
#protected-mode yes
################################### 常用配置 ####################################
# 设置端口号
port 6379
...
start() {
  docker run -p $PORT:6379 --name $SERVICE_NAME -v "$SERVICE_PATH"/redis.conf:/redis.conf -v "$SERVICE_PATH"/data:/data -itd --restart=always daocloud.io/library/redis redis-server /redis.conf
